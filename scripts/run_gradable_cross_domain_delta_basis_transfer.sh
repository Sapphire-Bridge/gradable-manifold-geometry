...
NORMALIZE_SOURCE_DELTAS="${NORMALIZE_SOURCE_DELTAS:-1}"
DRY_RUN="${DRY_RUN:-0}"
...
OUT_PREFIX_RAW="${OUT_PREFIX:-results/manifold_groups_poc/gradable_cross_domain_delta_basis_transfer_l${LAYERS//,/}_r${RANK}_gemma3}"
...
if [ "${NORMALIZE_SOURCE_DELTAS}" = "1" ]; then
  PATCH_ARGS+=(--normalize_source_deltas)
else
  PATCH_ARGS+=(--no-normalize_source_deltas)
fi
if [ "${DRY_RUN}" = "1" ]; then
  PATCH_ARGS+=(--dry_run)
fi
