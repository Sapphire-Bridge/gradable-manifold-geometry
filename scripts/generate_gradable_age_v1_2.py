    ...
    for value in VALUES:
        ...
        for i, left in enumerate(sides):
            for right in sides[i + 1 :]:
                ...
                base_key = f"age-v1_2-{value_tag}-{slug(left['context'])}-vs-{slug(right['context'])}"
                a, b, flipped = oriented_pair(left, right, pair_key=base_key)
                pair_id = f"age-v1_2-{value_tag}-{slug(a['context'])}-vs-{slug(b['context'])}"
                rows.append(
                    {
                        "pair_id": pair_id,
                        "target": str(a["target"]),
                        ...
                        "metadata": {
                            ...
                            "source": "bierwisch_kennedy_age_v1_2",
                        },
                    }
                )
    return rows
...
def main() -> None:
    ...
    out_jsonl = root / "gradable_age_disamb_pairs_v1_2.jsonl"
    out_csv = root / "gradable_age_grid_v1_2.csv"
    rows = build_items()
