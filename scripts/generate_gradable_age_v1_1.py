...
# (context, typical median lifespan in years for that species/role)
CONTEXTS = [
    ("a domestic mouse", 2),
    ("a domestic dog", 12),
    ("a horse", 28),
...
# scalar values in years
VALUES = [1, 3, 6, 10, 18, 30, 55, 90, 130, 180]


def label_for_ratio(ratio: float) -> str | None:
    # ratio = value / typical lifespan.
    # ratio <= 0.15 -> "young"; 0.15 < r <= 0.40 -> "youthful";
    # 0.55 <= r < 0.85 -> "mature"; r >= 0.85 -> "old"; gap 0.40<r<0.55 -> borderline.
    if ratio <= 0.15:
        return "young"
    if 0.15 < ratio <= 0.40:
        return "youthful"
...
def prompt_for(context: str, standard: int, value: int) -> str:
    return (
        f"The normal lifespan for {context} is {standard} years. "
        f"Compared only to that {standard}-year baseline for {context}, "
        f"an individual aged {value} years feels"
    )
    ...
    for value in VALUES:
        ...
        for i, a in enumerate(sides):
            for b in sides[i + 1 :]:
                ...
                pair_id = f"age-v1_1-{value_tag}-{slug(a['context'])}-vs-{slug(b['context'])}"
                rows.append(
                    {
                        "pair_id": pair_id,
                        "target": str(a["target"]),
                        ...
                        "metadata": {
                            ...
                            "source": "bierwisch_kennedy_age_v1_1",
                        },
                    }
                )
    return rows
...
def main() -> None:
    ...
    out_jsonl = root / "gradable_age_disamb_pairs_v1_1.jsonl"
    out_csv = root / "gradable_age_grid_v1_1.csv"
    rows = build_items()
