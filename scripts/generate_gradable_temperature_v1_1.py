    ...
    for value in VALUES:
        ...
        for i, a in enumerate(sides):
            for b in sides[i + 1 :]:
                ...
                pair_id = f"temp-v1_1-{value_tag}-{slug(a['context'])}-vs-{slug(b['context'])}"
                rows.append(
                    {
                        "pair_id": pair_id,
                        "target": str(a["target"]),
                        ...
                        "metadata": {
                            ...
                            "source": "bierwisch_kennedy_temperature_v1_1",
                        },
                    }
                )
    return rows
...
def main() -> None:
    ...
    out_jsonl = root / "gradable_temperature_disamb_pairs_v1_1.jsonl"
    out_csv = root / "gradable_temperature_grid_v1_1.csv"
    rows = build_items()
