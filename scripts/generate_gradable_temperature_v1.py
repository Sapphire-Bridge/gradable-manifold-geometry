...
def prompt_for(context: str, standard: int, value: int) -> str:
    return (
        f"For {context}, the normal temperature is {standard} degrees Celsius. "
        f"Relative to that standard, {value} degrees Celsius is"
    )
    ...
    for value in VALUES:
        ...
        for i, a in enumerate(sides):
            for b in sides[i + 1 :]:
                ...
                pair_id = f"temp-v1-{value_tag}-{slug(a['context'])}-vs-{slug(b['context'])}"
                rows.append(
                    {
                        "pair_id": pair_id,
                        "target": str(a["target"]),
                        ...
                        "metadata": {
                            ...
                            "source": "bierwisch_kennedy_temperature_v1",
                        },
                    }
                )
    return rows
...
def main() -> None:
    ...
    out_jsonl = root / "gradable_temperature_disamb_pairs_v1.jsonl"
    out_csv = root / "gradable_temperature_grid_v1.csv"
    rows = build_items()
